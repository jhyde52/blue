...
# base case:
# if more piles than objects (k > n), stop
# if piles equals object (k = n), return 1
# if one pile (k = 1), return 1
...
def stirling(n ,k):
    ...
    if n == k or k == 1:
        return 1
    return k * stirling(n-1, k) + stirling(n-1, k-1)  # recursive case


def bell(n):
    bell = 0
    for k in range(1, n + 1): # range stops one before end
        bell = bell + stirling(n,k)
    return bell

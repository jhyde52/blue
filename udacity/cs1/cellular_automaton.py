...
# 1. Pattern_number -> pattern The pattern numbers are powers of 2
# 2. Create a new string by applying the pattern to the string generation times
# 3. Return the last new string
...
def cellular_automaton(string, pattern_number, generations):
    ...
    for j in range(0, generations):
        new_string = ''
        for i in range(0, n):
            # python lets you index from reverse so if string[1] is 0, the one before will wrap around
            # added % because if i is last, gives us n%n which is zero, the 1st element

...
begin (your puzzle input). Each step is designated by a single letter. For example, suppose you have the
...
# Part one: FHMEQGIRSXNWZBCLOTUADJPKVY
#paradox 44
...
# heap is a tree where lowest is on the top, not sorted so no bottom
# or can have a heap that is a tree with highest on the top
# heapq built in library python  python software foundatio ---has a pushpop
# since only 26 letters would not have to do heap but it is more efficient
